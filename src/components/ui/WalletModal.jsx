...
if (typeof window !== 'undefined' && !window.__walletRegistry) {
  ...
  setTimeout(() => {
    if (window.ethereum && !reg.wallets.find(w => w.info.uuid === 'legacy')) {
      const name = window.ethereum.isRabby          ? 'Rabby'
                 ...
                 : 'Browser Wallet';
      reg.wallets.push({ info: { name, icon: null, uuid: 'legacy', rdns: 'legacy' }, provider: window.ethereum });
      reg.listeners.forEach(fn => fn([...reg.wallets]));
    }
  }, 300);
}
...
function useWalletRegistry() {
  const [wallets, setWallets] = useState(() =>
    typeof window !== 'undefined' ? [...(window.__walletRegistry?.wallets || [])] : []
  );
...
const EMOJI = {
  ...
  'Brave Wallet':'🦁', 'Trust Wallet':'🛡', 'Rainbow':'🌈',
  'Phantom':'👻', 'OKX Wallet':'⭕',
};
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', color:'var(--text-muted)', fontSize:24, cursor:'pointer', lineHeight:1, padding:'0 4px' }}>
            ×
          </button>
        </div>
        ...
        {wallets.length > 0 ? (
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {wallets.map(w => (
              <button key={w.info.uuid} onClick={() => handleConnect(w)} disabled={!!connecting}
                style={{
                  display:'flex', alignItems:'center', gap:14, padding:'13px 16px',
                  background:'var(--bg-elevated)', border:'1px solid var(--border)',
                  borderRadius:12, cursor:connecting ? 'not-allowed' : 'pointer',
                  opacity: connecting && connecting !== w.info.uuid ? 0.4 : 1,
                  transition:'border-color 0.15s', textAlign:'left', width:'100%',
                }}
                onMouseEnter={e => { if (!connecting) e.currentTarget.style.borderColor = 'var(--gold)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; }}
              >
                {w.info.icon
                  ? <img src={w.info.icon} alt={w.info.name} style={{ width:38, height:38, borderRadius:8 }} />
                  : <div style={{ width:38, height:38, borderRadius:8, background:'var(--bg)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>
                      {EMOJI[w.info.name] || '👛'}
                    </div>
                }
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:'0.9rem', fontWeight:600, color:'var(--text-primary)', fontFamily:'Syne, sans-serif' }}>
                    {w.info.name}
                  </div>
                ...
                <span style={{ fontSize:13, color: connecting === w.info.uuid ? 'var(--gold)' : 'var(--text-muted)', minWidth:70, textAlign:'right' }}>
          ...
          <div style={{ textAlign:'center', padding:'28px 0' }}>
            ...
            <div style={{ fontSize:'0.9rem', fontWeight:600, color:'var(--text-primary)', marginBottom:6 }}>No Wallet Detected</div>
            <div style={{ fontSize:'0.73rem', color:'var(--text-secondary)', marginBottom:20 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {[
                ...
                { name:'Coinbase', url:'https://wallet.coinbase.com', emoji:'🔵' },
              ].map(w => (
                <a key={w.name} href={w.url} target="_blank" rel="noopener noreferrer" style={{
                  display:'flex', alignItems:'center', gap:10, padding:'11px 14px',
                  background:'var(--bg-elevated)', border:'1px solid var(--border)',
                  borderRadius:10, color:'var(--text-primary)', textDecoration:'none', fontSize:'0.82rem',
                }}>
        ...
        {error && (
          <div style={{ marginTop:14, padding:'9px 13px', background:'rgba(239,68,68,0.1)',
            border:'1px solid rgba(239,68,68,0.3)', borderRadius:8, fontSize:'0.75rem', color:'#f87171' }}>
            ⚠ {error}
          </div>
        )}

        <div style={{ marginTop:16, fontSize:'0.62rem', color:'var(--text-muted)', textAlign:'center' }}>
