  ...
  async function execute() {
    ...
    try {
      ...
      if (err.code === 4001 || err.code === 'ACTION_REJECTED')
        ...
        toast.error('Deposit Failed', err.message?.slice(0, 100) || 'Transaction failed');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <div className="quick-picks">
            {QUICK_AMOUNTS.map(v => (
              <button key={v} className="quick-pick" onClick={() => setAmount(String(v))}>
                {v}
              </button>
            ))}
          </div>
        ...
        <div className="form-group">
          ...
          <div className="quick-picks">
            {QUICK_DURATIONS.map(({ label, days }) => (
              <button key={label} className="quick-pick" onClick={() => setUnlockVal(daysFromNow(days))}>
                {label}
              </button>
            ))}
          </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => !loading && setModalOpen(false)}>
        ...
        <div className="modal-detail">
          ...
          <div className="modal-detail-row">
            ...
            <span className="modal-detail-val">~0.0008 ETH</span>
          </div>
        </div>
        ...
        <div className="modal-actions">
          ...
          >
            {loading
              ? <><span className="spinner" /> {loading === 'mining' ? 'Mining…' : 'Confirm in Wallet…'}</>
              : 'Confirm & Lock'}
          </button>
        </div>
      </Modal>
