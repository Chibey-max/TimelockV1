...
import { withdraw, waitForReceipt, getVault, formatEther } from '@/lib/web3mini';
import { CONTRACT_ADDRESS, NETWORKS } from '@/lib/contract';
import { formatDate, truncateAddr } from '@/lib/utils';

export default function VaultList({
  vaults, isLoading, isConnected,
  address, chainId, ethPrice,
  onRefresh, onSuccess, toast,
}) {
  ...
  async function executeWithdraw() {
    ...
    setLoading(true);
    try {
      const txHash = await withdraw(address, CONTRACT_ADDRESS, withdrawVault.id);
      setWithdrawVault(null);
      toast.info('Transaction Submitted', `Withdrawing Vault #${withdrawVault.id}…`);
      ...
      if (err.code === 4001 || err.code === 'ACTION_REJECTED')
        ...
        toast.error('Withdrawal Failed', err.message?.slice(0, 100));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function showDetails(id) {
    try {
      ...
      toast.info(`Vault #${id}`, `${eth} ETH · Unlocks ${unlock} · ${v.isUnlocked ? '✅ Unlocked' : '🔒 Locked'}`);
    } catch (err) {
    ...
    <>
      <div className="card">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div className="card-title" style={{ marginBottom: 0 }}>Active Vaults</div>
        ...
        {!isConnected && (
          <div className="connect-prompt">
            ...
            <div className="connect-sub">Connect your wallet to view your vaults</div>
          </div>
        )}
      ...
      <Modal isOpen={!!withdrawVault} onClose={() => !loading && setWithdrawVault(null)}>
        ...
        <div className="modal-detail">
          <div className="modal-detail-row">
            ...
            <span className="modal-detail-val">{withdrawVault?.amount?.toFixed(4)} ETH</span>
          </div>
          <div className="modal-detail-row">
