...
function extractRevertReason(err) {
  ...
  const m = msg.match(/execution reverted: (.+)/i)
...
export async function deposit(from, contract, unlockTimeSec, weiAmount) {
  return sendTx(from, contract, SEL.deposit,
    encUint(unlockTimeSec), '0x' + weiAmount.toString(16));
}
...
export async function getVault(contract, userAddr, vaultId) {
  const raw = await ethCall(contract, SEL.getVault,
